...
import {
  ...
  generateLegacyGroupAltText,
  generateLegacyImageDescription,
...
export default function AdminRepairCasesPage() {
  ...
  function isAutoGeneratedImageDescription(
    value,
    currentForm,
    index,
  ) {
    const currentValue = normalizeImageSeoText(value);

    if (!currentValue) return true;

    const automaticValues = [
      generateImageDescription(currentForm, index),
      generateLegacyImageDescription(
        currentForm,
        index,
      ),
    ].map(normalizeImageSeoText);

    return automaticValues.includes(currentValue);
  }

  function isAutoGeneratedImageAlt(
    value,
    currentForm,
    description,
    index,
    ...
    const automaticValues = [
      generateGroupAltText(currentForm, index),
      generateLegacyGroupAltText(
        currentForm,
        index,
      ),
  ...
  function handleChange(e) {
    ...
    if (
      ["branch", "device", "model", "symptom"].includes(name)
    ) {
      setDetailImages((prevImages) =>
        prevImages.map((image, index) => {
          const descriptionWasAutomatic =
            isAutoGeneratedImageDescription(
              image.description,
              previousForm,
              index,
            );

          const altWasAutomatic =
            isAutoGeneratedImageAlt(
              image.alt_text,
              previousForm,
              image.description,
              index,
          ...
          const nextDescription = descriptionWasAutomatic
            ? generateImageDescription(nextForm, index)
            : image.description;

          const nextAlt = altWasAutomatic
            ? descriptionWasAutomatic
              ? generateGroupAltText(nextForm, index)
              : generateAltFromDescription(
                  nextForm,
                  nextDescription,
                  index,
    ...
    const mainAltWasAutomatic =
      !normalizeImageSeoText(previousForm.alt_text) ||
      normalizeImageSeoText(previousForm.alt_text) ===
        normalizeImageSeoText(makeAltText(previousForm));

    if (name !== "alt_text" && mainAltWasAutomatic) {
  ...
  async function uploadSingleFile(file, imageRole) {
    const contentType = String(file.type || "").toLowerCase();
    const uploadSlug = form.slug || makeSlug(form);
    ...
    if (!uploadResponse.ok) {
      throw new Error(
        `R2 이미지 업로드에 실패했습니다. (${uploadResponse.status})`,
      );
    }
  ...
  async function handleMainImageUpload(e) {
    ...
    try {
      const publicUrl = await uploadSingleFile(file, "main");
  ...
  async function handleDetailImagesUpload(e) {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        const publicUrl = await uploadSingleFile(
          files[i],
          "detail",
        );
      ...
      setMessage(`${files.length}장 상세 이미지 업로드 완료 · 자동 설명은 실제 사진에 맞게 확인해 주세요`);
    } catch (error) {
      console.error(error);
      setMessage(
        error.message ||
          "상세 이미지 업로드 중 오류가 발생했습니다.",
      );
    } finally {
      setUploading(false);
    }
  ...
  function handleDetailImageDescriptionChange(
    index,
    value,
  ) {
      ...
      return {
        ...
        alt_text: altWasAutomatic
          ? value.trim()
            ...
            : generateGroupAltText(form, index)
          : img.alt_text,
      };
    });
